    ...
    <Edit transform={transform}>
      <SimpleForm>
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
            ...
            <FormDataConsumer>
              {({ formData }) => {
                const formattedDate = formData.dateVisite
                  ? dayjs(formData.dateVisite).format('YYYY-MM-DD')
                  : null;
                const currentVisitSlot = getCurrentVisitSlot();
                ...
                return (
                  <>
                    ...
                    <input
                      ...
                      value={
                        selectedSlot
                          ? `${dayjs(selectedSlot.start).format('HH:mm')}-${dayjs(
                              selectedSlot.end,
                            ).format('HH:mm')}`
                          : currentVisitSlot
                          ? `${dayjs(currentVisitSlot.start).format('HH:mm')}-${dayjs(
                              currentVisitSlot.end,
                            ).format('HH:mm')}`
                          : ''
                      }
                    />
            ...
            <TextInput source="objet" label="Objet" validate={required()} fullWidth />
            <SelectInput
              source="status"
              choices={[
                { id: 'PLANIFIEE', name: 'Planifiée' },
